...
// 📌 Componente de item puro (React.memo impede que a lista pisque ou resete o scroll ao interagir)
const CardapioItem = React.memo(({ item, temEstoque, adicionarAoCarrinho }) => {
  const [expandido, setExpandido] = useState(false);

  // Sistema seguro: se o texto passar de 50 caracteres, ele libera a expansão
  const podeExpandir = item.info && item.info.length > 50;

  return (
    <View style={[styles.card, !temEstoque && styles.cardSemEstoque]}>
      
      {/* Camada Superior: Imagem e Nome */}
      <View style={styles.topoCard}>
        <View style={styles.containerImagem}>
          <Image source={item.imagem} style={styles.fotoComida} />
          {!temEstoque && (
      ...
      {/* Camada Inferior: Descrição e Botão Ver Mais */}
...
export default function CardapioScreen() {
  const [modalVisivel, setModalVisivel] = useState(MODO_DEV ? false : true);
    ...
    try {
      ...
      const produtosFormatados = dados.map(p => {
        ...
        if (p.img && typeof p.img === 'string' && p.img.trim() !== "") {
          // Remove pastas repetidas do caminho original
          const nomeArquivo = p.img
            .replace('/images/', '')
            .replace('/uploads/', '')
            .replace('images/', '')
            .replace('uploads/', '')
          ...
          // 🚀 encodeURIComponent trata perfeitamente espaços AND acentos (ex: Picolé, Strogonoff)
          imagemDefinida = { uri: `${URL_API}/images/${encodeURIComponent(nomeArquivo)}` };
        } else {
          imagemDefinida = require('../../assets/imagens/img_ntf.png');
        }
  ...
  // 📌 useMemo memoriza os filtros. Evita recalcular e jogar o Scroll pro topo sozinho!
  ...
  // 📌 useCallback impede recriação de funções que quebram o estado da FlatList
  ...
  return (
    <View style={styles.container}>
      ...
      {/* 📌 Abas com tamanho máximo de fonte (16px) e sem quebra ou scroll lateral */}
      ...
      <NotificacaoModal visivel={modalVisivel} fechar={() => setModalVisivel(false)} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  txtCategoria: { 
    ...
    fontSize: 16, // Aumentado para o tamanho máximo legível em 1 linha
    fontWeight: 'bold',
    textAlign: 'center'
  },
  txtCategoriaAtiva: { 
